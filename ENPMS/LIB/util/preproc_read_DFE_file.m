...
while ~feof(fileID)
   ...
   try
      ...
      % Take measurement value strings, remove any strings that say 'null', convert to double, and save as new column in array D
      MEASUREMENTS = str2double(strrep(fileData{4},'null',''));
      
      B = split(fileData{3},' ');  % Split the date-time cell column into separate cell columns
      E = cellstr(B(:,1));
      F = cellstr(strrep(B(:,2),':',''));
      fileData{3} = E; % Rejoin cell columns to the cell array 'D'
      fileData{4} = F;
      clear ('B','E','F')  % clear excess variables
      % this section provides alternative handling of the hour:minute
      % values
      DN = str2double(fileData{4});
